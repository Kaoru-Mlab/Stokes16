...
int main()
{
	...
	for(k = 0;k<end;k++){
		...
		for(s = 1; s <= s_end+1; s+=10){		//s+=はTAP_MAXと対応
			STEP_COEF = 1.0E-6;
			...
			for(loop = 0; loop < loop_end; loop++){
				...
				if(loop == loop_end - 1){
					Stokes_nomal(S_FIRed_DATA,S_FIRed_DATA_nomaled, DATA_SYMBOL);
					snprintf(filename,1024,"13_Sift_%d,TR_%d,step_%e_%.1fkm_%.1fdBm.csv",a,TR_SYMBOL,LMS_STEP,DIST_Z/1000.0,dBm);
					CSV_5(filename,DATA_SYMBOL,S_FIRed_DATA_nomaled);
				}
		...
		snprintf(filename,1024,"14_2_%dkm_%d_dBm,step_%e,BER_result.csv",(int)DIST_Z/1000,(int)dBm,STEP_COEF);
...
void EtoS(double *E_data, double *S_data){	//Ex,EyからStokes
	...
	for(i = 0; i < N_SYMBOL;i++)
	{
	S_data[4*i+0] = (E_data[4*i+0] * E_data[4*i+0] + E_data[4*i+1] * E_data[4*i+1]) + (E_data[4*i+2] * E_data[4*i+2] + E_data[4*i+3]*E_data[4*i+3]);
	S_data[4*i+1] = (E_data[4*i+0] * E_data[4*i+0] + E_data[4*i+1] * E_data[4*i+1]) - (E_data[4*i+2] * E_data[4*i+2] + E_data[4*i+3]*E_data[4*i+3]);
	S_data[4*i+2] = 2.0*(E_data[4*i+0] * E_data[4*i+2] + E_data[4*i+1] * E_data[4*i+3]);
	S_data[4*i+3] = 2.0*(E_data[4*i+0] * E_data[4*i+3] - E_data[4*i+1] * E_data[4*i+2]);
	}
