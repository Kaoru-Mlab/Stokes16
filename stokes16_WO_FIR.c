...
int main()
{
	...
	for(a = 0 ; a < 1 ; a++){					//変えた
		...
		for(k = 0;k<end;k++){
			...
			double* S_DELedT_NRMLed = (double*)malloc(sizeof(double)* DATA_SYMBOL *4);
			...
			for(loop = 0; loop < loop_end; loop++){
				...
				//データシンボルのみ取り出す
				DEL_TSYMBOL(S_parameter_rx);
					Stokes_nomal(S_parameter_rx,S_DELedT_NRMLed,DATA_SYMBOL);
					strcpy(filename,"12_rx_DataSymbol.csv");
					CSV_5(filename,DATA_SYMBOL,S_DELedT_NRMLed);
				...
				//ストークスからbit列に直し、エラービットを数える
				catch_Symbol(S_DELedT_NRMLed, rece_bit, DATA_SYMBOL);
