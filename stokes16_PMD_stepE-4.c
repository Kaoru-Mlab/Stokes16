	...
	for (i = 0; i < (int)SSFM_COUNT; i++){
		...
		if((int)now_distance % 100 == 0)//相関長0.1kmに設定(0.1kmごとに偏波回転する)
		{
  		for(j = 0; j < P_ANALOG; j++){
			...
			//群速度遅延
			O_data[4*j+0] = x_re * cos(0) + y_re * sin(0) * cos(phi[i]) - (y_im * sin(0) * sin(phi[i]));
			O_data[4*j+1] = x_im * cos(0) + y_im * sin(0) * cos(phi[i]) + y_re * sin(0) * sin(phi[i]);
			O_data[4*j+2] = y_re * cos(0) - (x_re * sin(0) * cos(phi[i])) - (x_im * sin(0) * sin(phi[i]));
			O_data[4*j+3] = y_im * cos(0) - (x_im * sin(0) * cos(phi[i])) + x_re * sin(0) * sin(phi[i]);
			...
			//偏波モード分散
			powX = sqrt(x_re * x_re + x_im * x_im);
			powY = sqrt(x_re * x_re + x_im * x_im);
			tmpX = powX; tmpY = powY;

			powX = powX * cos(theta[i]) + powY * sin(theta[i]);
			powY = powY * cos(theta[i]) + powX * sin(theta[i]);

			O_data[4*i +0] = (powX / tmpX) * x_re;
			O_data[4*i +1] = (powX / tmpX) * x_im;
			O_data[4*i +2] = (powY / tmpY) * y_re;
			O_data[4*i +3] = (powY / tmpY) * y_im;
...
int FIR_filter(double *S_aft, double *S_pre){
	...
	for(i=0; i < TR_SYMBOL; i++){
		...
		// 本信号のみに適応
	for (i = 0; i < N_SYMBOL; i++){ //試しに0からまわしてみる
		FIR_S[0] = S_aft[4*i +1];
		FIR_S[1] = S_aft[4*i +2];
		FIR_S[2] = S_aft[4*i +3];
		h1 = 0.0;
